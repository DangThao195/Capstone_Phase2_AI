...
# Review của anh Hiếu
## 1. Bucket thì nên đặt theo account id kiểu này: company-cdo-{account_id}-telemetry nếu account khác nhau còn ko thì chia namespace dạng này idempotency/cdo-01/ và idempotency/cdo-02/ nha.
 
## 2. ⁠Nếu CUR chưa chốt mà để telemetry_delay_event = true thì cần 2 field mới:
+ missing_resources: string[]
  Danh sách các line_item_product_code đã có trong Cost Explorer nhưng chưa có trong CUR. Sẽ giúp biết cần hold detection ở service nào.
+ curren_ce_cost_gap_usd: number
  Tổng cost (USD) của các services trong missing_resources theo CE - phần tiền AIO chưa nhìn thấy trong CUR. Sẽ biết rằng gap này đáng kể để cân nhắc không?
  
⁠## 3. Cần fix ở 2 chỗ: 
+ contract thì đổi traffic_volume sang must have thay vì optional - Do cần biết traffic tăng bao nhiêu để normalize cost⁠
+ Thêm feature cost_per_request trước khi đưa vào model:
  df['cost_per_request'] = df['daily_cost'] / df['traffic_volume’]. Anomaly hay không thì sẽ đánh giá dựa trên feature này chứ k phải cứ absolute cost tăng là có anomaly.